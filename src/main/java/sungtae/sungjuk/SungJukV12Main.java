...
public class SungJukV12Main {
    public static void main(String[] args) {
        ApplicationContext ctx =
                new ClassPathXmlApplicationContext("sungjuk12.xml");
        SungJukV11Service sjsrv = (SungJukV11ServiceImpl)ctx.getBean("sjsrv12");

        Scanner sc = new Scanner(System.in);
        String menu = "";

        while(true) { // 메뉴 띄우고 작업번호 입력을 반복
            ((SungJukV12ServiceImpl)sjsrv).displayMenu();
            menu = sc.nextLine();
